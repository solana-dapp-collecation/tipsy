...
type PostEntryResponse struct {
	...
	// Number of solanas the sender gave this post. Only set when getting solana posts.
	solanasFromSender uint64
}
...
func (fes *APIServer) _postEntryToResponse(postEntry *lib.PostEntry, addGlobalFeedBool bool, params *lib.BitCloutParams, utxoView *lib.UtxoView, readerPK []byte, verifiedMap map[string]*lib.PKID, maxDepth uint8) (
	*PostEntryResponse, error) {
	...
	res := &PostEntryResponse{
		...
		solanaCount:                   postEntry.solanaCount,
...
func (fes *APIServer) GetSinglePost(ww http.ResponseWriter, req *http.Request) {
	...
	sort.Slice(commentEntryResponseList, func(ii, jj int) bool {
		...
		// Next we sort based on solanas given by the poster.
		iiCommentEntry := commentHashHexToCommentEntry[iiCommentEntryResponse.PostHashHex]
		iisolanaKey := lib.MakesolanaKey(
			posterPKID.PKID,
			utxoView.GetPKIDForPublicKey(iiCommentEntry.PosterPublicKey).PKID,
			iiCommentEntry.PostHash)
		iisolanaLevelByPoster := utxoView.GetsolanaEntryForsolanaKey(&iisolanaKey)

		jjCommentEntry := commentHashHexToCommentEntry[jjCommentEntryResponse.PostHashHex]
		jjsolanaKey := lib.MakesolanaKey(
			posterPKID.PKID,
			utxoView.GetPKIDForPublicKey(jjCommentEntry.PosterPublicKey).PKID,
			jjCommentEntry.PostHash)
		jjsolanaLevelByPoster := utxoView.GetsolanaEntryForsolanaKey(&jjsolanaKey)

		if iisolanaLevelByPoster != nil && jjsolanaLevelByPoster == nil {
			// If ii received any solanas and jj did not receive any, ii is first.
			return true
		} else if iisolanaLevelByPoster == nil && jjsolanaLevelByPoster != nil {
			// If jj received any solanas and ii did not receive any, jj is first.
			return false
		} else if iisolanaLevelByPoster != nil && jjsolanaLevelByPoster != nil {
			// If both ii and jj received solanas, whichever received more solanas is placed first.
			// If they received an equal number of solanas, continue to the tiebreaker below on coin price.
			if iisolanaLevelByPoster.solanaLevel > jjsolanaLevelByPoster.solanaLevel {
				return true
			} else if iisolanaLevelByPoster.solanaLevel < jjsolanaLevelByPoster.solanaLevel {
				return false
			}
		}
...
type GetPostssolanaedBySenderForReceiverRequest struct {
	// Public key of the poster who received solanas from the sender
	ReceiverPublicKeyBase58Check string

	// Username of Receiver
	ReceiverUsername string

	// Public key of the sender who gave solanas to receiver
...
type GetPostssolanaedBySenderForReceiverResponse struct {
	// Map of solana level to a list of post entry responses ordered by timestamp
	solanaedPosts []*PostEntryResponse

	// Sum of all solanas sender gave to receiver
	TotalsolanasGiven uint64
...
func (fes *APIServer) GetsolanaedPosts(ww http.ResponseWriter, req *http.Request) {
	decoder := json.NewDecoder(io.LimitReader(req.Body, MaxRequestBodySizeBytes))
	requestData := GetPostssolanaedBySenderForReceiverRequest{}
	if err := decoder.Decode(&requestData); err != nil {
		_AddBadRequestError(ww, fmt.Sprintf(
			"GetsolanaedPosts: Problem parsing request body: %v", err))
		return
	}
	...
	utxoView, err := fes.backendServer.GetMempool().GetAugmentedUniversalView()
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: Error getting utxoView: %v", err))
		return
	}
	...
	if requestData.ReceiverPublicKeyBase58Check != "" {
		// Decode the receiver public key for which we are fetching posts that were solanaed.
		receiverPublicKeyBytes, _, err = lib.Base58CheckDecode(requestData.ReceiverPublicKeyBase58Check)
		if err != nil {
			_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: Problem decoding receiver public key: %v", err))
			return
		}
		...
		if receiverProfileEntry == nil {
			_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: No profile entry found for receiver username: %v", requestData.ReceiverUsername))
			return
		}
		receiverPublicKeyBytes = receiverProfileEntry.PublicKey
	} else {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: Neither ReceiverPublicKeyBase58Check nor ReceiverUsername provided"))
		return
	}
	...
	if requestData.SenderPublicKeyBase58Check != "" {
		// Decode the sender public key for which we are fetching posts that were solanaed.
		senderPublicKeyBytes, _, err = lib.Base58CheckDecode(requestData.SenderPublicKeyBase58Check)
		if err != nil {
			_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: Problem decoding sender public key: %v", err))
			return
		}
		...
		if senderProfileEntry == nil {
			_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: No profile entry found for sender username: %v", requestData.SenderUsername))
			return
		}
		senderPublicKeyBytes = senderProfileEntry.PublicKey
	} else {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: Neither SenderPublicKeyBase58Check nor SenderUsername provided"))
		return
	}
	...
	readerPublicKeyBytes, _, err := lib.Base58CheckDecode(requestData.ReaderPublicKeyBase58Check)
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: Problem decoding reader public key: %v", err))
		return
	}

	// Get the solanaEntries for this receiver-sender pair of public keys.
	solanaEntries, err := utxoView.GetsolanaEntriesForSenderToReceiver(receiverPublicKeyBytes, senderPublicKeyBytes)
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: Problem getting solana entries: %v", err))
		return
	}
	...
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf(
			"GetsolanaedPosts: Error fetching verifiedMap: %v", err))
		return
	}

	totalsolanasGiven := uint64(0)
	var solanaedPosts []*PostEntryResponse
	for _, solanaEntry := range solanaEntries {
		totalsolanasGiven += uint64(solanaEntry.solanaLevel)
		postEntry := utxoView.GetPostEntryForPostHash(solanaEntry.solanaPostHash)
		if postEntry != nil && !postEntry.IsDeleted() && !postEntry.IsHidden {
			...
			if err != nil {
				_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: Problem converting post entry to response: %v", err))
				return
			}
			postEntryReaderState := utxoView.GetPostEntryReaderState(readerPublicKeyBytes, postEntry)
			postEntryResponse.PostEntryReaderState = postEntryReaderState
			postEntryResponse.solanasFromSender = uint64(solanaEntry.solanaLevel)
			if postEntry.ParentStakeID != nil && len(postEntry.ParentStakeID) == lib.HashSizeBytes {
				...
				if parentPostEntry == nil {
					_AddBadRequestError(ww, fmt.Sprintf(
						"GetsolanaedPosts: Problem getting parent post with postHash %v for postEntry with hash %v",
				...
				if err != nil {
					_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: Problem converting parent post entry to response: %v", err))
				}
			...
			solanaedPosts = append(solanaedPosts, postEntryResponse)
		}
	}

	// Now sort posts by solana level then timestamp
	sort.Slice(solanaedPosts, func(ii, jj int) bool {
		postii := solanaedPosts[ii]
		postjj := solanaedPosts[jj]

		if postii.solanasFromSender > postjj.solanasFromSender {
			return true
		}
		if postii.solanasFromSender < postjj.solanasFromSender {
			return false
	...
	if startPostHashHex != "" || len(solanaedPosts) > numToFetch {
		startIndex := 0
		if startPostHashHex != "" {
			for ii, postEntryResponse := range solanaedPosts {
		...
		solanaedPosts = solanaedPosts[startIndex:lib.MinInt(startIndex+numToFetch, len(solanaedPosts)-1)]
	}

	res := &GetPostssolanaedBySenderForReceiverResponse{
		solanaedPosts:               solanaedPosts,
		TotalsolanasGiven:           totalsolanasGiven,
	...
	if err = json.NewEncoder(ww).Encode(res); err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanaedPosts: Problem encoding response as JSON: %v", err))
		return
	}
}
...
type GetsolanasForPostRequest struct {
	// PostHashHex to fetch.
	PostHashHex                string `safeForLogging:"true"`
	Offset                     uint32 `safeForLogging:"true"`
	Limit                      uint32 `safeForLogging:"true"`
...
type GetsolanasForPostResponse struct {
	solanaSenders []*solanaSenderResponse
}

type solanaSenderResponse struct {
	solanaSenderProfile *ProfileEntryResponse
	solanaLevel         int64
}

func (fes *APIServer) GetsolanasForPost(ww http.ResponseWriter, req *http.Request) {
	decoder := json.NewDecoder(io.LimitReader(req.Body, MaxRequestBodySizeBytes))
	requestData := GetsolanasForPostRequest{}
	if err := decoder.Decode(&requestData); err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanasForPost: Problem parsing request body: %v", err))
		return
	}

	postHash, err := GetPostHashFromPostHashHex(requestData.PostHashHex)
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanasForPost: %v", err))
		return
	}
	...
	if requestData.ReaderPublicKeyBase58Check != "" {
		...
		if err != nil {
			_AddBadRequestError(ww,
				fmt.Sprintf("GetsolanasForPost: Problem decoding user public key: %v : %s", err, requestData.ReaderPublicKeyBase58Check))
			return
		}
	}
	...
	utxoView, err := fes.backendServer.GetMempool().GetAugmentedUniversalView()
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanasForPost: Error constucting utxoView: %v", err))
		return
	}

	// Fetch the solanas for the post requested.
	pkidTosolanaLevel, err := utxoView.GetsolanaSendersForPostHash(postHash)
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanasForPost: Error getting pkidTosolanaLevel map %v", err))
		return
	}

	// Filter out any restricted profiles.
	pkMapToFilter := make(map[lib.PkMapKey][]byte)
	for senderPKID := range pkidTosolanaLevel {
	...
	filteredPkMap, err := fes.FilterOutRestrictedPubKeysFromMap(pkMapToFilter, readerPublicKeyBytes, "leaderboard" /*moderationType*/)
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanasForPost: Error filtering out restricted profiles: %v", err))
		return
	}

	// Create a list of unfiltered (aka not blacklisted) solanaSenders.
	solanaSenders := []*lib.ProfileEntry{}
	for senderPKID := range pkidTosolanaLevel {
		...
		if _, ok := filteredPkMap[lib.MakePkMapKey(profileEntry.PublicKey)]; ok {
			solanaSenders = append(solanaSenders, profileEntry)
		}
	}

	// Almost done. Just need to sort the comments.
	sort.Slice(solanaSenders, func(ii, jj int) bool {

		// Attempt to sort on bitclout locked.
		iiBitCloutLocked := solanaSenders[ii].BitCloutLockedNanos
		jjBitCloutLocked := solanaSenders[jj].BitCloutLockedNanos
		if iiBitCloutLocked > jjBitCloutLocked {
			return true
		} else if iiBitCloutLocked < jjBitCloutLocked {
			return false
		...
		// Attempt to sort on solana level.
		iiPKID := utxoView.GetPKIDForPublicKey(solanaSenders[ii].PublicKey)
		jjPKID := utxoView.GetPKIDForPublicKey(solanaSenders[jj].PublicKey)
		iisolanaLevel := pkidTosolanaLevel[*iiPKID.PKID]
		jjsolanaLevel := pkidTosolanaLevel[*jjPKID.PKID]
		if iisolanaLevel > jjsolanaLevel {
			return true
		} else if iisolanaLevel < jjsolanaLevel {
			return false
		}

		// Sort based on pub key if all else fails.
		return lib.PkToString(solanaSenders[ii].PublicKey, fes.Params) > lib.PkToString(solanaSenders[jj].PublicKey, fes.Params)
	})

	// Cut out the page of solanaSenders that we care about.
	solanaSendersLength := uint32(len(solanaSenders))
	// Slice the comments from the offset up to either the end of the slice or the offset + limit, whichever is smaller.
	maxIdx := lib.MinUint32(solanaSendersLength, requestData.Offset+requestData.Limit)
	solanaSendersPage := []*lib.ProfileEntry{}
	if solanaSendersLength > requestData.Offset {
		solanaSendersPage = solanaSenders[requestData.Offset:maxIdx]
	}

	// Grab verified username map pointer for constructing profile entry responses.
	verifiedMap, err := fes.GetVerifiedUsernameToPKIDMap()
	if err != nil {
		_AddInternalServerError(ww, fmt.Sprintf("GetsolanasForPost: Error fetching verifiedMap: %v", err))
		return
	}

	// Convert final page of solanaSenders to a list of solanaSender responses.
	solanaSenderResponses := []*solanaSenderResponse{}
	for _, solanaSender := range solanaSendersPage {
		solanaSenderPKID := utxoView.GetPKIDForPublicKey(solanaSender.PublicKey)
		solanaSenderResponse := &solanaSenderResponse{
			solanaSenderProfile: _profileEntryToResponse(solanaSender, fes.Params, verifiedMap, utxoView),
			solanaLevel:         pkidTosolanaLevel[*solanaSenderPKID.PKID],
		}
		solanaSenderResponses = append(solanaSenderResponses, solanaSenderResponse)
	}

	// Return the posts found.
	res := &GetsolanasForPostResponse{
		solanaSenders: solanaSenderResponses,
	}
	if err = json.NewEncoder(ww).Encode(res); err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanasForPost: Problem encoding response as JSON: %v", err))
		return
	}
}
