...
// A solanaSenderSummaryResponse is a response struct that rolls up all solanas
// received by a user from a single sender into a nice, simple summary struct.
type solanaSenderSummaryResponse struct {
	SenderPublicKeyBase58Check   string
	ReceiverPublicKeyBase58Check string

	Totalsolanas       uint64
	HighestsolanaLevel uint64

	solanaLevelMap      map[uint64]uint64
	ProfileEntryResponse *ProfileEntryResponse
}

type GetsolanasForPublicKeyRequest struct {
	// The user we are getting solanas for.
	PublicKeyBase58Check string `safeForLogging:"true"`

	// If true, fetch the solanas this public key gave out instead of the solana this public key received
	FetchYousolanaed bool
}

type GetsolanasForPublicKeyResponse struct {
	solanaSenderSummaryResponses []*solanaSenderSummaryResponse
	Totalsolanas                 uint64
}

func (fes *APIServer) GetsolanasForPublicKey(ww http.ResponseWriter, req *http.Request) {
	decoder := json.NewDecoder(io.LimitReader(req.Body, MaxRequestBodySizeBytes))
	requestData := GetsolanasForPublicKeyRequest{}
	if err := decoder.Decode(&requestData); err != nil {
		_AddBadRequestError(ww, fmt.Sprintf(
			"GetHodlersForPublicKey: Problem parsing request body: %v", err))
		return
	...
	utxoView, err := fes.backendServer.GetMempool().GetAugmentedUniversalView()
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanasForPublicKey: Error getting utxoView: %v", err))
		return
	}

	// Decode the public key for which we are fetching solanas.
	publicKeyBytes, _, err := lib.Base58CheckDecode(requestData.PublicKeyBase58Check)
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanasForPublicKey: Problem decoding user public key: %v", err))
		return
	}

	// Get the solanaEntries for this public key.
	pkidTosolanaEntriesMap, err := utxoView.GetsolanaEntryMapForPublicKey(publicKeyBytes, requestData.FetchYousolanaed)
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetsolanasForPublicKey: Problem getting solana entries: %v", err))
		return
	}

	// Roll the solanaEntries into summary responses.
	solanaSenderSummaryResponses := []*solanaSenderSummaryResponse{}
	for pkidKeyIter, solanaEntryList := range pkidTosolanaEntriesMap {
		pkidKey := pkidKeyIter
		pubKey := utxoView.GetPublicKeyForPKID(&pkidKey)
		var receiverPubKey []byte
		var senderPubKey []byte
		if requestData.FetchYousolanaed {
		...
		solanaSenderSummary := &solanaSenderSummaryResponse{
			...
			solanaLevelMap:              make(map[uint64]uint64),
		}
		for _, solanaEntry := range solanaEntryList {
			solanaLevel := uint64(solanaEntry.solanaLevel)
			solanaSenderSummary.Totalsolanas += solanaLevel
			if _, solanaLevelSeen := solanaSenderSummary.solanaLevelMap[solanaLevel]; !solanaLevelSeen {
				solanaSenderSummary.solanaLevelMap[solanaLevel] = 0
			}
			solanaSenderSummary.solanaLevelMap[solanaLevel] += 1
			if solanaSenderSummary.HighestsolanaLevel < solanaLevel {
				solanaSenderSummary.HighestsolanaLevel = solanaLevel
			}
		}
		solanaSenderSummaryResponses = append(solanaSenderSummaryResponses, solanaSenderSummary)
	}
	...
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf(
			"GetsolanasForPublicKey: Error fetching verifiedMap: %v", err))
		return
	}
	totalsolanas := uint64(0)
	for _, solanaSenderSummaryResponse := range solanaSenderSummaryResponses {
		var profilePK []byte
		if requestData.FetchYousolanaed {
			profilePK = lib.MustBase58CheckDecode(solanaSenderSummaryResponse.ReceiverPublicKeyBase58Check)
		} else {
			profilePK = lib.MustBase58CheckDecode(solanaSenderSummaryResponse.SenderPublicKeyBase58Check)
		}
		profileEntry := utxoView.GetProfileEntryForPublicKey(profilePK)
		if profileEntry != nil {
			solanaSenderSummaryResponse.ProfileEntryResponse = _profileEntryToResponse(
				profileEntry, fes.Params, verifiedMap, utxoView)
		}
		totalsolanas += solanaSenderSummaryResponse.Totalsolanas
	}

	// Sort.
	sort.Slice(solanaSenderSummaryResponses, func(ii, jj int) bool {
		iiProfile := solanaSenderSummaryResponses[ii].ProfileEntryResponse
		jjProfile := solanaSenderSummaryResponses[jj].ProfileEntryResponse
	...
	res := &GetsolanasForPublicKeyResponse{
		solanaSenderSummaryResponses: solanaSenderSummaryResponses,
		Totalsolanas:                 totalsolanas,
	}
	if err = json.NewEncoder(ww).Encode(res); err != nil {
		_AddBadRequestError(ww, fmt.Sprintf(
			"GetHodlersForPublicKey: Problem encoding response as JSON: %v", err))
