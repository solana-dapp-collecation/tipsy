...
const (
	...
	RoutePathSendsolanas             = "/api/v0/send-solanas"
	...
	RoutePathGetsolanasForPublicKey  = "/api/v0/get-solanas-for-public-key"
	...
	RoutePathGetsolanasForPost      = "/api/v0/get-solanas-for-post"
	...
	RoutePathGetsolanaedPosts       = "/api/v0/get-solanaed-posts"
...
func (fes *APIServer) NewRouter() *muxtrace.Router {
	var FrontendRoutes = []Route{
		...
		},
		{
			...
			[]string{"POST", "OPTIONS"},
			RoutePathGetsolanasForPublicKey,
			fes.GetsolanasForPublicKey,
			PublicAccess,
		},
		{
			"GetsolanaedPosts",
			[]string{"POST", "OPTIONS"},
			RoutePathGetsolanaedPosts,
			fes.GetsolanaedPosts,
			PublicAccess,
		},
		{
			"CreateNFT",
		...
		},
		{
			"GetsolanasForPost",
			[]string{"POST", "OPTIONS"},
			RoutePathGetsolanasForPost,
			fes.GetsolanasForPost,
			PublicAccess,
		},
		{
			"GetRecloutsForPost",
