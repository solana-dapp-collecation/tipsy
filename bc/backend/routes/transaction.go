...
func (fes *APIServer) SubmitTransaction(ww http.ResponseWriter, req *http.Request) {
	...
	_, solanaPostHashKeyExists := txn.ExtraData[lib.solanaPostHashKey]
	// If this is a basic transfer (but not a solana action), we check if user has completed the tutorial (if this node is configured for Jumio)
	if !solanaPostHashKeyExists && txn.TxnMeta.GetTxnType() == lib.TxnTypeBasicTransfer && fes.IsConfiguredForJumio() {
...
func (fes *APIServer) SubmitPost(ww http.ResponseWriter, req *http.Request) {
	...
	if requestData.InTutorial {
		...
		if userMetadata.TutorialStatus != solana {
			_AddBadRequestError(ww, fmt.Sprintf("SubmitPost: Must be in the GiveAsolanaComplete status in tutorial in order to post at this point in the tutorial: %v", err))
			return
		}
...
// SendsolanasRequest ...
type SendsolanasRequest struct {
	// The public key of the user who is making the transfer.
	SenderPublicKeyBase58Check string `safeForLogging:"true"`
	...
	// The number of solanas to give the post.
	solanaPostHashHex string `safeForLogging:"true"`

	// The number of solanas to give the post.
	solanaLevel int64 `safeForLogging:"true"`
...
// SendsolanasResponse ...
type SendsolanasResponse struct {
	SpendAmountNanos  uint64
	TotalInputNanos   uint64
	ChangeAmountNanos uint64
	FeeNanos          uint64
...
// Sendsolanas ...
func (fes *APIServer) Sendsolanas(ww http.ResponseWriter, req *http.Request) {
	decoder := json.NewDecoder(io.LimitReader(req.Body, MaxRequestBodySizeBytes))
	requestData := SendsolanasRequest{}
	if err := decoder.Decode(&requestData); err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("Sendsolanas: Problem parsing request body: %v", err))
		return
	}

	if requestData.SenderPublicKeyBase58Check == "" ||
		requestData.ReceiverPublicKeyBase58Check == "" ||
		requestData.solanaPostHashHex == "" {
		_AddBadRequestError(ww, fmt.Sprintf("Sendsolanas: Must provide a sender, a receiver, and a post hash to solana."))
		return
	}
	...
	if err != nil || len(senderPublicKeyBytes) != btcec.PubKeyBytesLenCompressed {
		_AddBadRequestError(ww, fmt.Sprintf(
			"Sendsolanas: Problem decoding sender public key %s: %v",
			requestData.SenderPublicKeyBase58Check, err))
		return
	}
	...
	if err != nil || len(receiverPublicKeyBytes) != btcec.PubKeyBytesLenCompressed {
		_AddBadRequestError(ww, fmt.Sprintf(
			"Sendsolanas: Problem decoding receiver public key %s: %v",
			requestData.ReceiverPublicKeyBase58Check, err))
		return
	}

	// Decode the solana post hash.
	solanaPostHashBytes, err := hex.DecodeString(requestData.solanaPostHashHex)
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf(
			"Sendsolanas: Problem decoding solanaPostHashHex %v: %v",
			requestData.solanaPostHashHex, err))
		return
	}
	if len(solanaPostHashBytes) != lib.HashSizeBytes {
		_AddBadRequestError(ww, fmt.Sprintf(
			"Sendsolanas: Invalid length for solanaPostHashHex %v",
			requestData.solanaPostHashHex))
		return
	}
	...
	if reflect.DeepEqual(senderPublicKeyBytes, receiverPublicKeyBytes) {
		_AddBadRequestError(ww, fmt.Sprintf("Sendsolanas: Sender and receiver cannot be the same."))
		return
	}

	// Try and create the transfer with solanas for the user.
	// We give solanas in CLOUT if we're past the corresponding block height.
	blockHeight := fes.blockchain.BlockTip().Height + 1
	var txn *lib.MsgBitCloutTxn
	var totalInput uint64
	var changeAmount uint64
	var fees uint64
	if blockHeight > lib.BitCloutsolanasBlockHeight {
		txn, totalInput, _, changeAmount, fees, err = fes.blockchain.CreateBasicTransferTxnWithsolanas(
			senderPublicKeyBytes,
			solanaPostHash,
			requestData.solanaLevel,
			// Standard transaction fields
			requestData.MinFeeRateNanosPerKB, fes.backendServer.GetMempool())
		if err != nil {
			_AddBadRequestError(ww, fmt.Sprintf("Sendsolanas: Problem creating transaction: %v", err))
			return
		}

	} else {
		txn, totalInput, changeAmount, fees, err = fes.blockchain.CreateCreatorCoinTransferTxnWithsolanas(
			senderPublicKeyBytes,
			receiverPublicKeyBytes,
			solanaPostHash,
			requestData.solanaLevel,
	...
	txnBytes, err := txn.ToBytes(true)
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("Sendsolanas: Problem serializing transaction: %v", err))
		return
	}
	...
	if requestData.InTutorial {
		...
		if err != nil {
			_AddBadRequestError(ww, fmt.Sprintf("Sendsolanas: Problem getting user metadata from global state: %v", err))
			return
		}
		if userMetadata.TutorialStatus != INVEST_SELF {
			_AddBadRequestError(ww, fmt.Sprintf("Sendsolanas: User should not be sending solanas at this point in the tutorial"))
			return
		}
		userMetadata.TutorialStatus = solana
		if err = fes.putUserMetadataInGlobalState(userMetadata); err != nil {
			_AddBadRequestError(ww, fmt.Sprintf("Sendsolanas: Problem putting user metadata in global state: %v", err))
			return
		}
	}
	...
	if err := json.NewEncoder(ww).Encode(res); err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("Sendsolanas: Problem encoding response as JSON: %v", err))
		return
	}
}
