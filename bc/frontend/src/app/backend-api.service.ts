...
export class BackendRoutes {
  ...
  static RoutePathGetsolanaedPosts = "/api/v0/get-solanaed-posts";
  ...
  static RoutePathSendsolanas = "/api/v0/send-solanas";
  static RoutePathGetsolanasForPublicKey = "/api/v0/get-solanas-for-public-key";
  static RoutePathGetLikesForPost = "/api/v0/get-likes-for-post";
  static RoutePathGetsolanasForPost = "/api/v0/get-solanas-for-post";
...
export class solanasPost {
  Post: PostEntryResponse;
  // Boolean that is set to true when this is the first post at a given solana level.
  ShowsolanaDivider?: boolean;
}
...
export class PostEntryReaderState {
  ...
  // Level of solana the user gave this post.
  solanaLevelBestowed?: number;
}
...
export class BackendApiService {
  ...
  GetsolanaedPosts(
    ...
    return this.post(endpoint, BackendRoutes.RoutePathGetsolanaedPosts, {
  ...
  Sendsolanas(
    ...
    SenderPublicKeyBase58Check: string,
    ReceiverPublicKeyBase58Check: string,
    solanaPostHashHex: string,
    solanaLevel: number,
    MinFeeRateNanosPerKB: number,
    InTutorial: boolean = false
  ): Observable<any> {
    const request = this.post(endpoint, BackendRoutes.RoutePathSendsolanas, {
      SenderPublicKeyBase58Check,
      ReceiverPublicKeyBase58Check,
      solanaPostHashHex,
      solanaLevel,
  ...
  GetsolanasForPublicKey(
    endpoint: string,
    PublicKeyBase58Check: string,
    FetchYousolanaed: boolean = false
  ): Observable<any> {
    return this.post(endpoint, BackendRoutes.RoutePathGetsolanasForPublicKey, {
      PublicKeyBase58Check,
      FetchYousolanaed,
    });
  }
  ...
  GetsolanasForPost(
    ...
    return this.post(endpoint, BackendRoutes.RoutePathGetsolanasForPost, {
      PostHashHex,
      Offset,
      Limit,
      ReaderPublicKeyBase58Check,
  ...
  parseProfileError(err): string {
    ...
    if (err && err.error && err.error.error) {
      ...
      if (errorMessage.indexOf("not sufficient") >= 0) {
        ...
        errorMessage = "You need more of your own creator coin to give a solana of this level.";
