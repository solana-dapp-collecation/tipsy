...
export class NotificationsListComponent {
  ...
  protected transformNotification(notification: any) {
    ...
    if (txnMeta.TxnType === "BASIC_TRANSFER") {
      ...
      if (basicTransferMeta.solanaLevel) {
        result.icon = "icon-solana fc-blue";
        let postText = "";
        ...
        result.action = `${actorName} gave <b>${basicTransferMeta.solanaLevel.toString()} solana${
          basicTransferMeta.solanaLevel > 1 ? "s" : ""
        }</b> (~${this.globalVars.getUSDForsolana(basicTransferMeta.solanaLevel)}) ${postText}`;
      } else {
      ...
      if (cctMeta.solanaLevel) {
        ...
        result.action = `${actorName} gave <b>${cctMeta.solanaLevel.toString()} solana${
          cctMeta.solanaLevel > 1 ? "s" : ""
        }</b> (~${this.globalVars.getUSDForsolana(cctMeta.solanaLevel)}) ${postText}`;
      } else {
