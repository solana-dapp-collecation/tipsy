...
const svgToProps = {
  [ConfettiSvg.solana]: { size: 10, weight: 1 },
...
export class GlobalVarsService {
  ...
  // Track if the user is dragging the solana selector. If so, need to disable text selection in the app.
  ...
  topsolanaedLeaderboard: LeaderboardResponse[] = [];
  ...
  // Map of solana level to bitclout nanos.
  solanaLevelMap = {};
  ...
  getUSDForsolana(index: number): string {
    const bitcloutNanos = this.solanaLevelMap[index];
  ...
  celebrate(svgList: ConfettiSvg[] = []) {
    ...
    if (svgList.length > 0) {
      ...
      if (svgList.indexOf(ConfettiSvg.solana) >= 0) {
  ...
  updateLeaderboard(forceRefresh: boolean = false): void {
    ...
    if (this.topsolanaedLeaderboard.length === 0 || forceRefresh) {
      pulseService.getsolanasReceivedLeaderboard().subscribe((res) => (this.topsolanaedLeaderboard = res));
    }
