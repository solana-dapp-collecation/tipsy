...
export class FeedPostIconRowComponent {
  @ViewChild("solanaPopover", { static: false }) solanaPopover: PopoverDirective;
  ...
  // Will need additional inputs if we walk through actions other than solanas.
  @Input() inTutorial: boolean = false;

  @Output() solanaSent = new EventEmitter();

  sendingRecloutRequest = false;

  // Threshold above which user must confirm before sending solanas
  static solanaWarningThreshold = 4;

  // Boolean for animation on whether a heart is clicked or not
  animateLike = false;

  solanaCount = 6;
  // Indexes from 0 to solanaCount (used by *ngFor)
  solanaIndexes = Array<number>(this.solanaCount)
    .fill(0)
    .map((x, i) => i);
  // Controls visibility of selectable solana levels. Initialize to false.
  solanasVisible = Array<boolean>(this.solanaCount).fill(false);
  // Store timeout functions so that they can be cancelled prematurely
  solanaTimeouts: NodeJS.Timer[] = [];
  // How quickly the solanas sequentially appear on hover
  solanaAnimationDelay = 50;
  // Where the drag div should be placed for mobile dragging
  solanaDragLeftOffset = "0px";
  // Whether the solana drag selector is being dragged
  solanaDragging = false;
  // Which solana is selected by the drag selector
  solanaIdxDraggedTo = -1;
  // Whether the drag selector is at the bottom of it's bound and in position to cancel a transaction
  solanaDragCancel = false;
  // Boolean for whether or not the div explaining solanas should be collapsed or not.
  collapsesolanaInfo = true;
  // Boolean for tracking if we are processing a send solanas event.
  sendingsolanas = false;
  // Track the solana selected in the solana popover.
  solanaSelected: number;
  // Track the solana that is currently being hovered
  solanaHovered = -1;
  // Track if we've gone past the explainer already. (Don't want to show explainer on start)
  solanaDragLeftExplainer = false;
  // Track if the dragged solana actually moved, so that we can distinguish between drags and clicks
  solanaDragMoved = false;
  // Track when the drag began, if less than .1 seconds ago, and the drag didn't move, assume it was a click
  solanaDragStarted: Date;
  ...
  // Initiate mobile drag, have solanas appear
  startDrag() {
    this.globalVars.userIsDragging = true;
    this.solanaDragMoved = false;
    this.solanaDragStarted = new Date();
    this.solanaDragging = true;
    this.addsolanaSelection({ type: "initiateDrag" });
  }
  ...
  duringDrag(event) {
    ...
    if (event.pointerPosition.x < pageMargin) {
      this.solanaIdxDraggedTo = 0;
      // If the selector is in the right margin, choose the last option
    } else if (event.pointerPosition.x > selectableWidth + pageMargin) {
      this.solanaIdxDraggedTo = this.solanaCount;
    } else {
      // If the selector is in the middle, calculate what % of the middle it has been dragged to, assign a solana value
      this.solanaIdxDraggedTo = round(((event.pointerPosition.x - pageMargin) / selectableWidth) * this.solanaCount);
    }
    ...
    if (this.solanaIdxDraggedTo != this.solanaCount) {
      this.solanaDragLeftExplainer = true;
    }
    // If the drag box is at the alloted lower boundry or below, set confirm status to true
    this.solanaDragCancel = event.distance.y > 30;
  }

  // Triggered on end of a touch. If we determine this was a "click" event, send 1 solana. Otherwise nothing
  dragClick(event) {
    const now = new Date();
    // If the drag box wasn't moved and less than 200ms have transpired since the start of the tap,
    // assume this was a click and send 1 solana
    if (!this.solanaDragMoved) {
      if (now.getTime() - this.solanaDragStarted.getTime() < 200) {
        // Prevent touch event from propagating
        event.preventDefault();
        this.sendOnesolana(event, true);
      }
      // If the solana drag box wasn't moved, we need to reset these variables.
  ...
  endDrag(event) {
    ...
    this.solanaDragging = false;
    // If the drag box is not in the "cancel" position, and the selected solana makes sense, send solanas
    if (!this.solanaDragCancel && this.solanaIdxDraggedTo > -1 && this.solanaIdxDraggedTo < this.solanaCount) {
      this.onsolanaSelected(null, this.solanaIdxDraggedTo);
    }
  ...
  resetDragVariables() {
    ...
    this.solanaDragCancel = false;
    this.solanaDragging = false;
    this.solanaIdxDraggedTo = -1;
    this.solanaDragMoved = false;
    this.solanaDragLeftExplainer = false;
  }
  ...
  toggleExplainer(event) {
    event.stopPropagation();
    this.collapsesolanaInfo = !this.collapsesolanaInfo;
  }

  sendsolanas(solanas: number, skipCelebration: boolean = false): Promise<void> {
    this.sendingsolanas = true;
    return this.backendApi
      ...
      .then(
        (res) => {
          this.sendingsolanas = false;
          this.globalVars.logEvent("solana: send", {
            ...
            solanaPostHashHex: this.postContent.PostHashHex,
            solanaLevel: solanas,
          });
          this.solanaSelected = solanas;
          this.postContent.solanaCount += solanas - this.getCurrentsolanaLevel();
          this.postContent.PostEntryReaderState.solanaLevelBestowed = solanas;
          if (!skipCelebration) {
            // Celebrate when the Sendsolanas call completes
            this.globalVars.celebrate([ConfettiSvg.solana]);
          }
          this.globalVars.updateEverything(res.TxnHashHex, this.sendsolanasSuccess, this.sendsolanasFailure, this);
        },
        (err) => {
          if (err.status === 0) {
          ...
          this.globalVars.logEvent("solanas: send: error", { parsedError });
          this.globalVars._alertError(parsedError);
        }
      );
  }

  sendsolanasSuccess(comp: FeedPostIconRowComponent) {
    comp.sendingsolanas = false;
    comp.solanaSent.emit(null);
  }

  sendsolanasFailure(comp: FeedPostIconRowComponent) {
    comp.sendingsolanas = false;
  ...
  popoverOpenClickHandler = (e: Event) => {
    const popoverElement = document.getElementById("solana-popover");
    if (popoverElement && e.target !== popoverElement && !popoverElement.contains(e.target as any)) {
      e.stopPropagation();
    }
  };

  async sendOnesolana(event: any, fromDragEvent: boolean) {
    // Disable solana selection if solanas are being sent
    if (this.sendingsolanas) {
      return;
    }

    // Block user from selecting solana level below already gifted amount
    if (this.getCurrentsolanaLevel() > 0) {
      return;
    }

    // Don't trigger solana purchases on tap on tablet
    ...
    this.onsolanaSelected(event, 0);
  }

  addsolanaSelection(event) {
    // Need to make sure hover event doesn't trigger on child elements
    if (event?.type === "initiateDrag" || event.target.id === "solana-button") {
      for (let idx = 0; idx < this.solanaCount; idx++) {
        this.solanaTimeouts[idx] = setTimeout(() => {
          this.solanasVisible[idx] = true;
        }, idx * this.solanaAnimationDelay);
      }
    }
  }

  removesolanaSelection() {
    for (let idx = 0; idx < this.solanaCount; idx++) {
      clearTimeout(this.solanaTimeouts[idx]);
      this.solanasVisible[idx] = false;
    }
  }

  async onsolanaSelected(event: any, index: number): Promise<void> {
    if (!this.globalVars.loggedInUser?.PublicKeyBase58Check) {
      this.globalVars._alertError("Must be logged in to send solanas");
      return;
    }
    // Disable solana selection if solanas are being sent
    if (this.sendingsolanas) {
    ...
    // Block user from selecting solana level below already gifted amount
    if (index < this.getCurrentsolanaLevel()) {
      return;
    }

    if (index + 1 <= this.postContent.PostEntryReaderState.solanaLevelBestowed) {
      this.globalVars._alertError("You cannot downgrade a solana");
      return;
    }
    this.solanaSelected = index + 1;
    if (event) {
      event.stopPropagation();
    }
    if (this.solanaSelected > FeedPostIconRowComponent.solanaWarningThreshold) {
      SwalHelper.fire({
        target: this.globalVars.getTargetComponentSelector(),
        icon: "info",
        title: `Sending ${this.solanaSelected} solanas to @${this.postContent.ProfileEntryResponse?.Username}`,
        html: `Clicking confirm will send ${this.globalVars.getUSDForsolana(
          this.solanaSelected
        ...
        if (res.isConfirmed) {
          await this.sendsolanas(this.solanaSelected);
        }
      });
    } else {
      await this.sendsolanas(this.solanaSelected);
  ...
  getCurrentsolanaLevel(): number {
    return this.postContent.PostEntryReaderState?.solanaLevelBestowed || 0;
  }

  getPopoverContainerClass() {
    const mobileClass = this.globalVars.isMobile() ? "solana-popover-container-mobile " : "";
    return "solana-popover-container " + mobileClass;
  }
}
